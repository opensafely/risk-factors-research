...
* Open a log file
capture log close
log using "./output/an_univariable_cox_models_workingagepop_`outcome'_`firstvar'TO`lastvar'", text replace
...
foreach var of any `varlist' {
	...
	*Fit and save model
	...
	capture stcox `model' if agegroup<=4, strata(stp) 
	if _rc==0 {
		estimates
		estimates save ./output/models/an_univariable_cox_models_workingagepop_`outcome'_AGESEX_`var', replace
		}
