...
* Open a log file
capture log close
log using "./output/an_bp_exploration_cox_models_`outcome'_`firstvar'TO`lastvar'", text replace
...
foreach var of any `varlist' {
	...
	*Fit and save model
	...
	if _rc==0 {
		estimates
		estimates save ./output/models/an_bp_exploration_cox_models_`outcome'_AGESEX_`var', replace
		}
